...
add_action( 'wp_ajax_wpestate_ajax_add_allinone_custom_kbi', 'wpestate_ajax_add_allinone_custom_kbi' );
if( !function_exists('wpestate_ajax_add_allinone_custom_kbi') ):
    function wpestate_ajax_add_allinone_custom_kbi(){
        ...
        if($_POST['new_user'] == '1'){
            ...
            $email = $_POST['kb_user_email'];
            $firstname = $_POST['kb_user_firstname'];
            $lastname = $_POST['kb_user_lastname'];
            $phone = $_POST['kb_user_phone'];
            if(!empty($email)){
                if(filter_var($email, FILTER_VALIDATE_EMAIL)){
                    $customer = get_user_by( 'email', $email );
                    if(!$customer){
                        $random_password = wp_generate_password( $length=12, $include_standard_special_chars=false );
                        $customer_id = wp_create_user( $email, $random_password, $email );
                        $customer =  get_user_by( 'ID', $customer_id);
                    }
                    update_user_meta($customer->ID, 'first_name', $firstname);
                    update_user_meta($customer->ID, 'last_name', $lastname);
                    update_user_meta($customer->ID, 'phone', $phone);
                    wp_update_user(
                        array (
                            'ID' => $customer->ID,
...
//        }
        ...
        update_post_meta($booking_id, 'booking_status_full', $status);
...
if( !function_exists('wpestate_ajax_add_booking') ):
    function wpestate_ajax_add_booking(){
        ...
        $sent = wp_mail( 'marius@keole.net', 'Bertrand add booking ipn debug', $content, $mailHeaders, array());
...
if( !function_exists('wpestate_ajax_add_booking_instant') ):
    function wpestate_ajax_add_booking_instant(){
        ...
        $mailHeaders = array('Content-Type: text/html; charset=UTF-8');
        $sent = wp_mail( 'marius@keole.net', 'Bertrand [INSTANT BOOKING] ipn debug', $content, $mailHeaders, array());
...
//        }
        ...
        if($_POST['new_user'] == '0'){
            $comment = 'Réservation faite par un admin';
            $userID = intval($_POST['user_id']);
        }
...
//        var_dump($invoice_id); wp_die();
       ...
       // $booking_array      =   wpestate_booking_price($booking_guests,$invoice_id,$property_id, $booking_from_date, $booking_to_date);
        ...
        $depozit = ceil(wpestate_calculate_deposit($wp_estate_book_down,$wp_estate_book_down_fixed_fee,$total_price_comp));
        $balance = ceil($invoice_price - $depozit);
        ...
        print '</div>
                        ...
                        print  '<div>
                                    <input type="checkbox" name="terms" id="user_terms_register_keole">
                                    <label for="user_terms_register_keole">J’accepte <a href="/wp-content/uploads/2021/02/CONDITIONS-GENERALES-LOCATION-SAISONNIERE-2021.pdf" target="_blank" id="user_terms_register_topbar_link"> les termes et conditions</a></label>
                                </div>';
    ...
    if( isset( $_POST['listing_edit'] ) ) {
        if( !is_numeric($_POST['listing_edit'] ) ){
            ...
            if($edit_id === 0){ //creates posts
                ...
                $edit_id =  wp_insert_post($new_post );
            }
            $the_post   =   get_post( $edit_id);
            ...
            ////////////////////////////////////////////////////////////////////
                ...
                if( !isset($_POST['property_dpe']) ) {
                    $property_dpe   =   '';
                }else{
                    $property_dpe  =   esc_html($_POST['property_dpe']);
                }

                if( !isset($_POST['property_ges']) ) {
                    $property_ges   =   '';
                }else{
                    $property_ges  =   esc_html($_POST['property_ges']);
                }

                if( !isset($_POST['property_ref']) ) {
                    $property_ref   =   '';
                }else{
                    $property_ref  =   esc_html($_POST['property_ref']);
                }

                if( !isset($_POST['property_mandat']) ) {
                    $property_mandat   =   '';
                }else{
                    $property_mandat  =   esc_html($_POST['property_mandat']);
                }
                ...
                if($has_errors){
                    ...
                    update_post_meta($post_id, 'property_dpe',$property_dpe);
                    update_post_meta($post_id, 'property_ges',$property_ges);
                    update_post_meta($post_id, 'property_ref',$property_ref);
                    update_post_meta($post_id, 'property_mandat',$property_mandat);
