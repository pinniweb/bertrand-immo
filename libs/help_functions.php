...
function wpestate_show_booking_form($post_id,$wpestate_options='',$favorite_class='',$favorite_text='',$is_shortcode=''){
    ...
    <div class="booking_form_request is_shortcode<?php echo intval($is_shortcode); ?> <?php echo esc_attr($container_class);?>" id="booking_form_request">
        ...
        <?php // if(is_user_logged_in()): ?>
            ...
            <input type="hidden" name="booking_guest_no"  value="<?=isset($_GET['guest_no_prop']) && $_GET['guest_no_prop']!='' ? esc_html( $_GET['guest_no_prop']) : 'all'?>">
        ...
        <?php // else: ?>
            <!-- <div class="wpb_btn-info wpb_btn-small wpestate_vc_button  vc_button" style="cursor: pointer;width: 100%;font-weight: 600;text-align: center" id="kbi_booking_form_login">Se connecter</div> -->
        <?php // endif; ?>
                ...
                <div class="third-form-wrapper">
                    ...
                    <div class="col-md-6 reservation_buttons">
                        <a style="width: 100%;padding: 10px;cursor: pointer;text-align: center;border: 2px solid #bbc0cb;color: #8A8F9A;display: inline-block;font-size: 14px;font-weight: 600;-webkit-transition: all 0.3s ease;-moz-transition: all 0.3s ease;-o-transition: all 0.3s ease;transition: all 0.3s ease;border-radius: 2px;" href="/contacter">
                            Contacter l'agence
                        </a>
                    </div>
                </div>
...
function wpestate_print_property_unit_slider($post_id,$wpestate_property_unit_slider,$wpestate_listing_type,$wpestate_currency,$wpestate_where_currency,$link,$return_type=''){
    ...
    if (count($post_attachments)) {
        $preview            =    wp_get_attachment_image_src($post_attachments[0]->ID, 'wpestate_property_listings');
        ...
        print '<div class="listing-unit-img-wrapper">';
                ...
                if(  $wpestate_property_unit_slider=='yes'){
                    ...
                    for ($i = 1; $i < count($post_attachments); $i++) {
                        $no_slides++;
                        $preview    =   wp_get_attachment_image_src($post_attachments[$i]->ID, 'wpestate_property_listings');
                    ...
                    print  '<div class="price_unit_wrapper"> </div>';
                        ...
                        print'<div class="price_unit">';
                            ...
                            $categTerms = get_the_terms( $post_id, 'property_category' );

                            switch ($categTerms[0]->slug ){
                                case 'location' :
                                    echo '<span class="pernight"> / Mois</span>';
                                break;
                                case 'vente' : break;
                                case 'location-saisonniere' :
                                    echo '<span class="pernight"> / Semaine</span>';
                                break;
                            }
...
function wprentals_redux_on_save($value,$value2){
    ...
    if( isset( $value['wp_estate_adv_search_type'] ) && ( $value['wp_estate_adv_search_type'] =='newtype' || $value['wp_estate_adv_search_type'] =='oldtype') ){
            ...
            $adv_search_what_classic_half    =   array('Location','check_in','check_out','guest_no','property_rooms','property_category','property_action_category','property_ref', 'property_mandat', 'property_bedrooms','property_bathrooms','property_price');
...
function wpestate_show_price($post_id,$wpestate_currency,$wpestate_where_currency,$return=0){
    ...
    if( !empty($custom_fields) && isset($_COOKIE['my_custom_curr']) &&  isset($_COOKIE['my_custom_curr_pos']) &&  isset($_COOKIE['my_custom_curr_symbol']) && $_COOKIE['my_custom_curr_pos']!=-1){
        ...
        if ($price != 0) {
            ...
            $categTerms = get_the_terms( $post_id, 'property_category' );
            if ($property_operation === 'vente' || ($property_operation === 'location' && $categTerms[0]->slug === 'location')) {
                $price = $price;
            } else {
                $price = 7 * $price;
            }
            // $price      = wpestate_TrimTrailingZeroes($price);
            if ($wpestate_where_currency == 'before') {
                $price = $wpestate_currency . ' ' . number_format($price, 0, ',', ' ');
            } else {
                $price = number_format($price, 0, ',', ' '). ' ' . $wpestate_currency;
            }
...
if( !function_exists('kbi_wpestate_show_price') ):
function kbi_wpestate_show_price($post_id,$wpestate_currency,$wpestate_where_currency,$return=0){
    ...
    switch ($categTerms[0]->slug){
        case 'vente' :
            print  trim($property_price_before_label.' '.$price.' €');
            break;
        case 'location' :
            print  '<span style="font-size: 28px;">'.trim($property_price_before_label.' '.$price.' €').'</span>';
            print '<span class="pernight_label" style="margin-left: 12px;display: flex; flex-direction: column; align-items: center;"><span style="font-size: 12px;">par mois</span><span style="font-size: 12px;margin-top: -8px;">charges comprises</span></span>';
            break;
        case 'location-saisonniere' :
            $price      =   floatval( get_post_meta($post_id, 'property_price', true) );
            $price      = number_format(7 * $price, 0, '.', ' ');
            print '<div style="display: flex; flex-direction: column;"><div style="font-size:13px">À partir de</div><div>'.$price.' €</div></div>';
            print '<span style="margin-left:16px;" class="pernight_label"> par semaine</span>';
            break;
    }
}
endif;
