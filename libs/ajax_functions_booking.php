...
if( !function_exists('wpestate_add_booking_invoice') ):
    function wpestate_add_booking_invoice(){
        ...
        $sent = wp_mail( 'marius@keole.net', 'Bertrand [INSTANT BOOKING INVOICE] ipn debug', $content, $mailHeaders, array());
...
if( !function_exists('wpestate_direct_confirmation') ):
    function wpestate_direct_confirmation(){
        ...
        $mailHeaders = array('Content-Type: text/html; charset=UTF-8');
        $sent = wp_mail( 'marius@keole.net', 'Bertrand [CONFIRMATION BOOKING] ipn debug', $content, $mailHeaders, array());
...
if( !function_exists('wpestate_ajax_show_booking_costs') ):
    function wpestate_ajax_show_booking_costs(){
        ...
        $total_price = ceil($booking_array['total_price'] - $booking_array['security_deposit']);
        $advance_price = ceil($total_price * 25 / 100);
        $balance_price = $total_price - $advance_price;
        $price_show                         =   wpestate_show_price_booking($booking_array['default_price'],$wpestate_currency,$wpestate_where_currency,1);
        $total_price_show                   =   wpestate_show_price_booking($total_price,$wpestate_currency,$wpestate_where_currency,1);
        ...
        if (array_key_exists('numberDays', $booking_array)) {
            $weeks = intdiv($booking_array['numberDays'], 7);
        }
        ...
        <div class="show_cost_form" id="show_cost_form" >
            <div class="cost_row">
                ...
                if($booking_array['price_per_guest_from_one']==1){
                    ...
                    if( $booking_array['has_custom'] == 1 ){
                        print esc_html($booking_array['numberDays']).' nuits';
                    }else if( $booking_array['has_wkend_price']===1 && $booking_array['cover_weekend']===1) {
                        print esc_html($booking_array['numberDays']).' nuits';
                    }else{
                        print esc_html(strval($weeks)).($weeks === 1 ? ' semaine' : ' semaines');
                        // print  esc_html($price_show).' x '.$booking_array['numberDays'].' nuits';
                    }
...
//                $numberOfWeeks = intval($booking_array['numberDays']/7);
//                $priceWeeks = intval($booking_array['inter_price']/$numberOfWeeks);
//
//        print '
//                <div class="cost_row">
//                    <div class="cost_explanation">'.$priceWeeks.' € x '.$numberOfWeeks.' semaine'.($numberOfWeeks >1?'s':'').'</div>
//                    <div class="cost_value">'.$inter_price_show.'</div>
//                </div>';
        ...
        print '
                <div class="cost_row" id="total_cost_row">
                    ...
                    <div class="cost_value" data_total_price="'.$advance_price.'" >'.$total_price_show.'</div>
                </div>
            </div>';
        
        print '
            <div style="margin-bottom: 16px;" class="cost_row" id="advance_cost_row">
                <div class="cost_explanation"><strong>Acompte (25%)</strong></div>
                <div class="cost_value">'.$advance_price.'€</div>
            </div>
        </div>';
        ...
        if($instant_booking==1){
            ...
            print '<span class="instant_depozit_value">'.$advance_price.'€</span>';
            ...
            if(floatval($booking_array['balance'])!=0){
                print '<div class="cost_row_instant instant_balance">'.esc_html__( 'Balance remaining','wprentals').': <span class="instant_balance_value">'.$balance_price.'€</span></div>';
            }
            ...
            print'<div class="instant_book_info" data-total_price="'.$advance_price.'" data-deposit="'.$advance_price.'" data-balance="'.$balance_price.'"> ';
        }
