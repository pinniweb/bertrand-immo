...
<div class="listing_main_image header_masonry panel-body imagebody imagebody_new" id="">
        ...
        $total_pictures=count ($post_attachments);
            ...
            for ($i = 1; $i < count($post_attachments); $i++) {
                ...
                if($count <= 3 && $count !=0){
                    $full_prty          = wp_get_attachment_image_src($post_attachments[$i]->ID, 'listing_full_slider');
                ...
                $full_prty          = wp_get_attachment_image_src($post_attachments[$i]->ID, 'wpestate_property_full_map');
                $hidden.= ' <a href="'.esc_url($full_prty[0]).'" rel="data-fancybox-thumb" data-fancybox="website_rental_gallery" title="'.esc_attr($post_attachments[$i]->post_excerpt).'" data-caption="'.esc_attr($post_attachments[$i]->post_excerpt).'" class="fancybox-thumb prettygalery listing_main_image" >
                        <img  src="'.esc_url($full_prty[0]).'" data-original="'.esc_attr($full_prty[0]).'" alt="'.esc_attr($post_attachments[$i]->post_excerpt).'" class="img-responsive " />
                    </a>';
            }
