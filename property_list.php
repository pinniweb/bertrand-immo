...
if (!empty(get_query_var('property_action_category'))) {
    $categ_term = get_term_by('slug', get_query_var('property_action_category'), 'property_action_category');
    $categ_array=array(
        'taxonomy'  => 'property_action_category',
        'field'     => 'slug',
        'terms'     => get_query_var('property_action_category')
    );
    $current_adv_filter_search_label= $categ_term->name;
}
...
if (!empty(get_query_var('property_category'))) {
    $action_term = get_term_by('slug', get_query_var('property_category'), 'property_category');
    $action_array=array(
        'taxonomy'  => 'property_category',
        'field'     => 'slug',
        'terms'     => get_query_var('property_category')
    );
    $current_adv_filter_category_label= $action_term->name;
}
...
if (!empty(get_query_var('property_city'))) {
    $city_term = get_term_by('slug', get_query_var('property_city'), 'property_city');
    $city_array=array(
        'taxonomy'  => 'property_city',
        'field'     => 'slug',
        'terms'     => get_query_var('property_city')
    );
    $current_adv_filter_city_label= $city_term->name;
}
...
if(!empty($_GET['property_bedrooms'])){
    $compare_array=array();
    $compare_array['key']        = 'property_bedrooms';
    $compare_array['value']      = $_GET['property_bedrooms'];
    $compare_array['type']       = 'string';
    $compare_array['compare']    = '>=';
    $meta_query[]                = $compare_array;
}

if(!empty($_GET['property_mandat'])) {
    $args = array(
        ...
        'meta_query' => array(
            array(
                'key' => 'property_mandat',
                'value' => $_GET['property_mandat'],
            )
        )
     );
     $posts = get_posts($args);
     if(count($posts) > 0) {
        $location = $posts[0]->guid;
