...
jQuery(document).ready(function ($) {
    ...
    if (jQuery('#property_description').length > 0) {
        var quill = new Quill('#property_description', {
            modules: {
                toolbar: '#toolbar_property_description'
            },
            theme: 'snow',
            placeholder: 'Saisir la description du bien...'
        });
    }
...
function wpestate_create_payment_action() {
  ...
  // $("#instant_booking_modal .woo_pay")

  jQuery('#user_terms_register_keole').on('change', function () {
    jQuery('.woo_pay').toggleClass('kactive')
  })

    jQuery(document).on('click', '.woo_pay.kactive',function () {
