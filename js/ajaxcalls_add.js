...
jQuery(document).ready(function ($) {
    ...
    $('#kbi_allinone_booking_existing_customer').on('click',function(event){
        $('#kbi_dates').removeClass('hidden')
        $('#kbi_reservation_set_booking_for_user').removeClass('hidden')
        $('#kbi_reservation_set_booking_for_new').addClass('hidden')
        $('#kbi_allinone_booking_new_customer').removeClass('active')
        $(this).addClass('active')
    });

    $('#kbi_allinone_booking_new_customer').on('click',function(event){
        $('#kbi_dates').removeClass('hidden')
        $('#kbi_reservation_set_booking_for_new').removeClass('hidden')
        $('#kbi_reservation_set_booking_for_user').addClass('hidden')
        $('#kbi_allinone_booking_existing_customer').removeClass('active')
        $(this).addClass('active')
    });

    $( '#kbi_booking_user' ).select2( {
    ...
    $('#allinone_set_custom_for_user').on('click',function(event){
        $('#allinone_set_custom').text(ajaxcalls_vars.saving);
        wpestate_check_booking_valability_internal_allinone(0);
    });

    $('#allinone_set_custom_for_new').on('click',function(event){
        $('#allinone_set_custom').text(ajaxcalls_vars.saving);
        wpestate_check_booking_valability_internal_allinone(1);
    });
...
function wpestate_check_booking_valability_internal_allinone(new_user) {
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            if ( data.trim() === 'run') {
                wpestate_allin_one_owner_insert_book_internal(new_user);
...
function wpestate_allin_one_owner_insert_book_internal(new_user) {

    var fromdate, todate, listing_edit, nonce, ajaxurl, comment, booking_guest_no,hour_from,hour_to, lastname, firstname, email, tel;
    ajaxurl             =   control_vars.admin_url + 'admin-ajax.php';
    fromdate            =   jQuery("#start_date_owner_book").val();
    todate              =   jQuery("#end_date_owner_book").val();
    listing_edit        =   jQuery('#listing_edit').val();
    comment             =   jQuery("#book_notes").val();
    lastname            =   jQuery("#kbi_user_lastname").val();
    firstname           =   jQuery("#kbi_user_firstname").val();
    email               =   jQuery("#kbi_user_email").val();
    tel                 =   jQuery("#kbi_user_tel").val();
    ...
    var user_id =   jQuery("#kbi_booking_user").val();
    ...
    jQuery.ajax({
        ...
        url: ajaxurl,
        data: {
            'action'            :   'wpestate_ajax_add_allinone_custom_kbi',
            'fromdate'          :   fromdate,
            'todate'            :   todate,
            ...
            'user_id'           :   user_id,
            'kb_user_email'     :   email,
            'kb_user_firstname' :   firstname,
            'kb_user_lastname'  :   lastname,
            'kb_user_phone'     :   tel,
            'allinone'          :   1
        },
        success: function (data) {
    ...
    $('.booking-calendar-wrapper-allinone .has_future').on('click',function (event) {
        ...
        if (($(this).hasClass('calendar-reserved') && !$(this).hasClass('start_reservation') ) || $(this).hasClass('pick_block_dates')) { // click on a booked spot
            return;
        }else{
    ...
    $('#form_submit_1').on('click',function () {
        ...
        var property_affiliate,security,ajaxurl,title,prop_category,prop_action_category,property_city,property_area_front,property_country,property_description,guest_no,new_estate,instant_booking, property_dpe,property_ges;
        ...
        property_dpe        = jQuery('#property_dpe').val();
        property_ges        = jQuery('#property_ges').val();
        ...
        $.ajax({
            ...
            data: {
                ...
                'property_dpe'          :  property_dpe,
                'property_ges'          :  property_ges,
                'security'              :  security
            },
            success: function (data) {
...
//       / submit_change();
    ...
    $('#edit_prop_1').on('click',function () {
        var ajaxurl, title, category, action_category, guests, city, country, area,listing_edit,prop_desc,property_admin_area,instant_booking,aff_link,private_notes, property_dpe,property_ges, property_ref, property_mandat;
        ...
        listing_edit    =  jQuery('#listing_edit').val();
        prop_desc       =  jQuery('#property_description')[0].firstElementChild.innerHTML;
        property_admin_area = jQuery ('#property_admin_area').val();
        private_notes=  jQuery('#private_notes').val();

        property_dpe=  jQuery('#property_dpe').val();
        property_ges=  jQuery('#property_ges').val();
        property_ref=  jQuery('#property_ref').val();
        property_mandat= jQuery('#property_mandat').val();
        ...
        $.ajax({
            ...
            data: {
                ...
                'property_mandat'   :   property_mandat,
                'security'          :   nonce
            },
            success: function (data) {
