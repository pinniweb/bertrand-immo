...
function wpestate_booking_show_booked(date){
    ...
    if (jQuery('#' + from_who).val()) {
        start_date = jQuery('#' + from_who).val().split('-')
        start_date = new Date(start_date[1] + '-' + start_date[0] + '-20' + start_date[2])
    ...
    if( booking_array[unixtime1] != undefined){
        ...
        if(start_reservation_class==1){
            ...
            if (!is_reserved && start_date && (start_date < unixtime)) {
                is_reserved = true
            }
        ...
        if (is_reserved) {
            return "disabled wpestate_calendar"+reservation_class+" date"+unixtime1_key+" "+from_css_class;    
        }
...
function wpestate_booking_form_currency_convert_back(display_price){
    ...
    $('#submit_booking_front,#submit_booking_front_instant').on('click',function (event) {
        ...
        if(property_vars.logged_in==="no"  && is_woo){
            $('#booking_form_request_mess').show().empty().addClass('book_not_available').append(property_vars.notlog);
            login_modal_type=3;
