...
function wpestate_start_filtering_ajax_map(newpage,pan_ne_lat, pan_ne_long, pan_sw_lat,pan_sv_long,move_map) {
    ...
    if(jQuery('#property_category_vente').length){
      jQuery.each(ajaxcalls_vars.adv_search_kbi_vente, function( index, element ) {
        if(index==='property_price_vente'){
          all_fields.push({element:'price_low',value: parseInt(jQuery('#price_low_vente').val(), 10)});
          all_fields.push({element:'price_max',value: parseInt(jQuery('#price_max_vente').val(), 10) });
        }else{
          all_fields.push({element:element.toLowerCase(),value:jQuery('#'+index.toLowerCase()).val() });
        }
      });
    }

    if(jQuery('#property_category_loc').length){
      jQuery.each(ajaxcalls_vars.adv_search_kbi_loc, function( index, element ) {
        if(index==='property_price_loc'){
          all_fields.push({element:'price_low',value: parseInt(jQuery('#price_low_loc').val(), 10)});
          all_fields.push({element:'price_max',value: parseInt(jQuery('#price_max_loc').val(), 10) });
        }else{            
          all_fields.push({element:element.toLowerCase(),value:jQuery('#'+index.toLowerCase()).val() });
        }
      });
    }
    if(jQuery('#property_category_loc_saison').length){
        console.log('search loc');
      jQuery.each(ajaxcalls_vars.adv_search_kbi_loc_saison, function( index, element ) {
        console.log(jQuery('#'+index.toLowerCase()).val())
        if(index==='property_price_loc_saison'){
          all_fields.push({element:'price_low',value: parseInt(jQuery('#price_low_loc_saison').val(), 10)});
          all_fields.push({element:'price_max',value: parseInt(jQuery('#price_max_loc_saison').val(), 10) });
        }else{
          all_fields.push({element:element.toLowerCase(),value:jQuery('#'+index.toLowerCase()).val() });
        }
      });
    ...
    if(jQuery('#property_city_vente').length >0){
        city                    =   jQuery('#property_city_vente').val();
    }
    if(jQuery('#property_city_loc').length >0){
        city                    =   jQuery('#property_city_loc').val();
    }
    if(jQuery('#property_city_loc_saison').length >0){
        city                    =   jQuery('#property_city_loc_saison').val();
    }
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            var  new_markers = kbi_price_format_array(jQuery.parseJSON(data.markers));
            if (infoBox !== null) {
                infoBox.close();
            }
...
function kbi_price_format_array(array){
    var newArray = [];
    jQuery.each(array, function( index, object ) {
        var newObject = object;
        switch (object[6]){
          case 'location' :
            newObject[5] = newObject[20] + ' / Mois'
            break;
          case 'vente' : break;
          case 'location-saisonniere' :
            newObject[5] = newObject[20] + ' / Jour'
            break;
        }
...
function wpestate_start_filtering(newpage) {
    ...
    var action, category, city, area, order, ajaxurl, mandat, page_id;
    // get action vars
    ...
    // get mandat
    mandat = jQuery('#a_filter_mandat').val();
    ajaxurl =  ajaxcalls_vars.admin_url + 'admin-ajax.php';
...
jQuery(document).ready(function ($) {
    ...
    /* $('.woo_pay.kactive').on('click',function () {
        var pay_paypal, prop_id, book_id, invoice_id, is_featured, is_upgrade,ajaxurl,depozit,is_submit;
        prop_id     =   $(this).attr('data-propid');
        depozit     =   $(this).attr('data-deposit');
        is_featured =   $(this).attr('data-is_featured');
        is_upgrade  =   0;
        is_submit   =   1;
